...
if (btnSubmitSearch) {
    btnSubmitSearch.onclick = () => {
        ...
        } else if (btnSubmitSearch.classList.contains('btn-search-driver')) {
            searchDriver(searchType, searchContent);
...
function searchDriver(searchType, searchContent) {
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            render(this.responseText);
        }
    ...
    xhttp.open('GET', '/admin/driver/search/'+searchType+'/'+searchContent, true);
    xhttp.send();
